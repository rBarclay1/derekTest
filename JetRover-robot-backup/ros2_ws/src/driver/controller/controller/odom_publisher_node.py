...
class Controller(Node):
    ...
    def cmd_vel_callback(self, msg):
        ...
        if self.machine_type == 'JetRover_Mecanum':
            ...
            self.angular_z = msg.angular.z
            speeds = self.mecanum.set_velocity(self.linear_x*(self.wheel_diameter_mecanum/self.wheel_diameter_tank), 0.0, self.angular_z)
            self.motor_pub.publish(speeds)
            ...
            if speeds[0] is not None:
                servo_state.position = [1, int(speeds[0])]
