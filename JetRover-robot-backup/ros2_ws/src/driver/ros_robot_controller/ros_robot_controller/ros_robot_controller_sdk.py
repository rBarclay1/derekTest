...
class PacketFunction(enum.IntEnum):
    # 可通过串口实现的控制功能(control functions that can be implemented through serial communication)
    PACKET_FUNC_SYS = 0
    PACKET_FUNC_LED = 1  # LED控制(LED control)
    PACKET_FUNC_BUZZER = 2  # 蜂鸣器控制(buzzer control)
    PACKET_FUNC_MOTOR = 3  # 电机控制(motor control)
    PACKET_FUNC_PWM_SERVO = 4  # PWM舵机控制, 板子上从里到外依次为1-4(PWM servo control, from inside to outside on the board are 1-4)
    PACKET_FUNC_BUS_SERVO = 5  # 总线舵机控制(bus servo control)
    PACKET_FUNC_KEY = 6  # 按键获取(obtain button)
    PACKET_FUNC_IMU = 7  # IMU获取(obtain MU)
    PACKET_FUNC_GAMEPAD = 8  # 手柄获取(obtain joystick)
    PACKET_FUNC_SBUS = 9  # 航模遥控获取(obtain aircraft model remote control)
...
class Board:
    ...
    def __init__(self, device="/dev/rrc", baudrate=1000000, timeout=5):
        ...
        # 队列用来存储数据(queues are used to store data)
    ...
    def get_button(self):
        # 获取按键key1， key2状态，返回按键ID(1表示按键1，2表示按键2)和状态(0表示按下，1表示松开)(Get the status of key1 and key2, return the key ID (1 for key1, 2 for key2) and the status (0 for pressed, 1 for released))
        if self.enable_recv:
            try:
    ...
    def get_imu(self):
        # 获取IMU数据，返回ax, ay, az, gx, gy, gz(get IMU data, and return ax, ay, az, gx, gy, gz)
        if self.enable_recv:
            try:
                # ax, ay, az, gx, gy, gz
    ...
    def set_oled_text(self, line, text):
        data = [line, len(text)] # 子命令为 0x01 设置 SSID, 第二个字节是字符串长度，该长度包含'\0'字符串结束符(Subcommand 0x01 is used to set the SSID. The second byte is the length of the string, which includes the '\0' string termination character)
